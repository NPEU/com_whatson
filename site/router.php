 ...
 * WhatsOn Router
 */
class WhatsOnRouter extends JComponentRouterBase
{
    /**
     * Build the route for the com_whatson component
     *
    ...
    public function build(&$query)
    {
        ...
        if (
            !empty($query['view']) && $query['view'] == 'whatson'
         && !empty($query['Itemid'])
        ) {
            unset ($query['view']);
            return $segments;
        ...
        if (
            isset($query['view']) && $query['view'] == 'entry'
        ...
        if (isset($query['task']) && $query['task'] == 'entry.add') {
            $segments[] = 'add';
            unset ($query['task']);
        }

        if (isset($query['task']) && isset($query['id']) && $query['task'] == 'entry.edit') {
            $segments[] = 'edit';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
        ...
        if (isset($query['task']) && isset($query['id']) && $query['task'] == 'entry.view') {
            $segments[] = 'view';
            $segments[] = $query['id'];
            unset ($query['task']);
            unset ($query['id']);
 ...
 */
function whatsonBuildRoute(&$query)
{
    $router = new WhatsOnRouter;
 ...
 * @deprecated  4.0  Use Class based routers instead
 */
function whatsonParseRoute($segments)
{
    $router = new WhatsOnRouter;
